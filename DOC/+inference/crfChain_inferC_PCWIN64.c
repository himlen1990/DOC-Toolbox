...
double logZ;
    ...
    tmp=(double*)calloc(edgePot.numColumns, sizeof(double));
	...
	//edgeBelDims = { nStates, nStates, nNodes - 1 };
    edgeBelDims[0] = nStates;
    edgeBelDims[1] = nStates;
    edgeBelDims[2] = nNodes - 1;
    ...
    tmpA=(double*)calloc(alpha.numColumns, sizeof(double));
	...
	for (n = nNodes - 1; n >= 1; --n) {
		...
		for (i = 1; i <= beta.numColumns; ++i) {
			tmp1 += submat_value(beta, n, i);
		}
		for (i = 1; i <= beta.numColumns; ++i) {
			submat_value(beta, n, i) /= tmp1;
		}
	}

	for (n = 1; n <= nNodes; ++n) {
		//tmpA[alpha.numColumns];
		...
		sumTmp = 0;
		for (i = 1; i <= alpha.numColumns; ++i) {
			tmpA[i-1] += submat_value(alpha, n, i)*submat_value(beta, n, i);
			sumTmp += tmpA[i-1];
		}

		for (i = 1; i <= alpha.numColumns; ++i) {
			submat_value(nodeBel, n, i) = tmpA[i-1]/sumTmp;
		}
	}
